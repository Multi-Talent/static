...
jQuery(document).ready(function () {
    ...
    var scColor1 = "#428BCA";
    if (
        jQuery.cookie("change-skin") &&
        jQuery.cookie("change-skin") == "bluenav"
    ) {
        scColor1 = "#fff";
    }
    ...
    jQuery(".menutoggle").click(function () {
        ...
        if (bodypos != "relative") {
            ...
            else body.addClass("leftpanel-show");
    ...
    function reposition_topnav() {
        if (jQuery(".nav-horizontal").length > 0) {
            ...
            if (jQuery(".nav-horizontal").css("position") == "relative") {
                ...
                jQuery(".nav-horizontal .children").removeClass(
                    "dropdown-menu"
                );
    ...
    if (c) {
        jQuery("head").append(
            '<link id="skinswitch" rel="stylesheet" href="' + cssSkin + '" />'
        );
    }
    ...
    if (fnt) {
        jQuery("head").append(
            '<link id="fontswitch" rel="stylesheet" href="css/font.' +
                fnt +
                '.css" />'
        );
    }
...
function get_file_name_from_rs_header(headerstr) {
    if (headerstr === null) {
...
function ms_save_or_open_blob(url, open_type, wait_message, file_name, content_type, send_option) {
    show_cover(wait_message);

    var xhr = new XMLHttpRequest();
    xhr.open(open_type, url , true);
    ...
    xhr.onreadystatechange = function() {
        ...
        if (xhr.readyState === 4 && xhr.status === 200) {
            if (typeof(file_name) === "undefined"  || file_name === "") {
                ...
                file_name = get_file_name_from_rs_header(cd);
            }
            // IEの場合
    ...
    if (typeof(send_option) !== "undefined"  && send_option !== "") {
        xhr.send(send_option);
    } else {
        xhr.send();
    }
}