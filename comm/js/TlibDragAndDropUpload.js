...
function init_drop_file_target() {
    ...
    // ファイルアップロード前にチェックを行う
    var checkBeforeUpload = function(fileTypeId, chk_flg) {
        // FormDataオブジェクトを用意
        var fd = create_form_data(args['func']['check_before_upload'], fileTypeId);
        ...
        $.ajax({
            ...
            contentType: false,
            success: function(result) {
                post_check_before_upload_by_drop(result, fileTypeId, chk_flg);
            },
            error: function(xmlhttprequest, textstatus, errorThrown) {
                hide_loading_msg();
                alert_dlg("Error", "Error at " + err_func_name + "<br>\n" +
    ...
    var post_check_before_upload_by_drop = function (result, fileTypeId, chk_flg) {
        if(result.status === 'Error') {
            hide_loading_msg();
            show_error_message(result.msg);
            return false;
        }
        confirmUploadFiles(fileTypeId, chk_flg);
    }
    ...
    var onDropFile = function(evt) {
        ...
        // ファイル一括アップロードの場合
            ...
            if(args['func']['check_before_upload'] !== undefined) {
                checkBeforeUpload(fileTypeId, true);
            } else {
    ...
    $("." + args['elm_ids']['input_class']).change(function(){
        ...
        // ファイル一括アップロードの場合
            ...
            if(args['func']['check_before_upload'] !== undefined) {
                checkBeforeUpload(fileTypeId, false);
            } else {
